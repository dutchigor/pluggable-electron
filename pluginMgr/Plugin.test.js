...
beforeAll(() => {
	...
	if (!existsSync(testPluginDir)) mkdirSync(testPluginDir)
...
afterAll(() => {
	...
	if (existsSync(testPluginDir)) rmSync(testPluginDir, { recursive: true })
})
...
describe('install', () => {
	...
	it('should throw an error and the plugin should be set to inactive if plugin does not contain any activation points', async () => {
		...
		await expect(() => plugin._install()).rejects.toThrow(
			'The plugin does not contain any activation points'
		)
