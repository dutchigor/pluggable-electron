...
beforeAll(async () => {
	...
	if (!existsSync(activePluginDir)) mkdirSync(activePluginDir)
	writeFileSync(
		activeManifest,
		JSON.stringify({
	...
	if (!existsSync(inactivePluginDir)) mkdirSync(inactivePluginDir)
	writeFileSync(
		inactiveManifest,
		JSON.stringify({
...
afterAll(() => {
	...
	if (existsSync(activePluginDir)) rmSync(activePluginDir, { recursive: true })
	if (existsSync(inactivePluginDir)) rmSync(inactivePluginDir, { recursive: true })
})
