/**
 * @typedef {Object} Extension Reference to a function or object that can be called by an extension point
...
class ExtensionPoint {
   ...
   * @param {string} name See {@link Extension}
   * @param {Object|Callback} response See {@link Extension} 
   * @param {number} priority See {@link Extension}
   * @returns {void}
   */
  ...
  /**
   * Execute (if callback) or return (if object) the response for each extension registered to this extension point.
  ...
  execute( input, exitOnError = true ) {
    ...
    return Promise[fn]( this.#extensions.map( p => {
